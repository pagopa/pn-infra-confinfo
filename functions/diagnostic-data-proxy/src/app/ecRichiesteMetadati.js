 ...
 * @return {Promise<Object>} The event data associated with the requestId, if
 * found.
 */
...
/**
  * Retrieves events from DynamoDB based on a requestId. If `checkRetry` is
  * enabled, it attempts to fetch additional events by appending a .PCRETRY_0,
  * .PCRETRY_1 etc.
  *
  * @param {string} requestId - The base requestId for the event(s) to fetch.
  * @param {boolean} checkRetry - A flag to determine whether to perform
  * retries. If true, the function will attempt to fetch events with .PCRETRY_i.
  * @return {Promise<Array>} A promise that resolves to an array of events
  * related to the requestId. If no events are found it returns an empty array.
  */
